# -*- coding: utf-8 -*-
"""
Created on Wed Mar  6 02:13:59 2019
    ...
    while cap.isOpened():
        ...
        if len(faceRects) > 0:
            for faceRect in faceRects:
                ...
                image = frame[y:y+h+250, x-10:x+w+10] # 将当前帧含人脸部分保存为图片，注意这里存的还是彩色图片，前面检测时灰度化是为了降低计算量；这里访问的是从y位开始到y+h-1位
...
if __name__ =='__main__':
    ...
    if os.path.exists(path):
        shutil.rmtree(path,True)        #若为文件夹，则删除该文件夹及文件夹内所有文件
        print("dir "+str(path)+" removed!")
    os.mkdir(path)
    getTrainingData('getTrainData',0,path,100) # 注意这里的training_data_xx 文件夹就在程序工作目录下